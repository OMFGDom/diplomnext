...
interface InputFieldProps {
	...
	onChange?: ChangeEventHandler<HTMLInputElement> // Добавление обработчика onChange
}
	...
	(
		{
			...
			isNumber,
			onFocus,
			onChange, // Добавление пропса onChange
			...rest
		},
		ref
	) => {
		return (
			<div className={`${extra}`}>
				<label
					htmlFor={id}
					className='text-sm text-black text-[16px]'
				>
					{label}
				</label>
				<input
					...
					onFocus={onFocus}
					onChange={onChange} // Использование пропса onChange
